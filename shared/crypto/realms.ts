...
type AccountKey = ChildKey<typeof Purpose.Account>;
type AppKey = ChildKey<typeof Purpose.App>;
type RealmKey = ChildKey<typeof Purpose.Realm>;
...
export async function deriveAcctKey(parent: MasterSeed, acctKDM: IKDM): Promise<ValStat<AccountKey>> {
  return deriveKey({ parent, purpose: Purpose.Account, kdm: acctKDM });
}
...
export async function deriveAppKey(parent: AppKey, appKDM: IKDM): Promise<ValStat<AppKey>> {
  return deriveKey({ parent, purpose: Purpose.App, kdm: appKDM });
}
...
export async function deriveRealmKey(parent: AppKey | AccountKey, realmKDM: IKDM): Promise<ValStat<RealmKey>> {
  return deriveKey({ parent, purpose: Purpose.Realm, kdm: realmKDM });
}
